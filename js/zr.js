    ...
    Utils = $.extend(Utils,{
        ...
        checkIsLoadingStatus:function(modNames,runtime){
             ...
             $.each(modNames,function(i,n){
                 var r;
                 //check css directly pass
                 if($.endWidth(n,".css")){
                     r = 0;
                 }else{
                     r = runtime.global.MODULES[n] || 0;
                 }
...
//
    ...
    $.extend(moduleLoader.prototype,{
        ...
        calculate:function(modNames, cache, ret){
            ...
            //
            $.each(modNames,function(i,n){
                mod = utils$1.createModuleInfo(runtime,n);
                modStatus = mod.status;
                m = utils$1.extModnames([n]);
                if(modStatus >= runtime.Loader.Status.READY_TO_ATTACH){
                    //再次判定当前是否存在
                    waitingModules.remove(m[0]);
                    //remove this
        ...
        checkFromModuleList:function(modNames){
            var config$$1 = this.runtime.baseConfig,
                mod,mdname;
            $.each(modNames,function(i,n){
                mod = utils$1.extModnames([n]);
                mdname = config$$1.module[mod];
                if(mdname){
                    modNames[i] = mdname.path;
                }
            });
        ...
        eachLoadModules:function(normalizedModNames){
            ...
            $.each(cssArray,function(a,b){
                //
                if(Zr$1.global.MODULES[b].status == Zr$1.Loader.Status.ATTACHED){
                    index++;
                    return;
                }
                //
                ...
                getFile(moduleUrl,function(){
                               ...
                               //css载入后直接更改为已绑定依赖状态
                               Zr$1.global.MODULES[b].status = Zr$1.Loader.Status.ATTACHED;
                           });
            });
            //
    ...
    $.extend(ModulesObject,{
        Modules:function(){
            ...
            finalCallback = function(){
                callback.apply(Zr$1,utils$1.getModules(Zr$1,modules));
            };
            //根据模块名称进行修改 => 获取正确模块名称
            modules = utils$1.extModnames(modules);
            //
            //
            function loadReady(){
                ++tryCount;
                ...
                ret = utils$1.checkModsLoadRecursively(modules,Zr$1);
                Zr$1.log(tryCount + " check duration :" + (+(new Date()) - start)+"ms");
                if(ret){
                    // console.log("检测模块是否依赖");
                    utils$1.attachModsRecursively(modules,Zr$1);
                }else{
            ...
            loader = moduleLoader(Zr$1,waitingModules);
            //
        ...
        ModulesFix:function(){
            ...
            modules = utils$1.checkChildStringToArray(modules);
            //根据模块名称进行修改 => 获取正确模块名称
            var extModules = utils$1.extModnames(modules);
            //根据模块名，获取正确的命名规则
            modNames = utils$1.checkFromModuleList(extModules,Zr$1);
            modNames = utils$1.extModnames(modNames);
            //将结果集返回到回到函数体内, 此函数只有在依赖载入完毕,模块功能生效时调用
            finalCallback = function(){
                callback.apply(Zr$1,utils$1.getModules(Zr$1,modNames));
            };
            //
            function loadReady(){
                ++tryCount;
                ...
                ret = utils$1.checkModsLoadRecursively(modNames,Zr$1);
                Zr$1.log(tryCount + " check duration :" + (+(new Date()) - start)+"ms");
                if(ret){
                    //因为需要的模块已经成功载入，但是还没有检查该模块是否有需要依赖的模块
                    modObj = utils$1.checkRequireModules(modNames,Zr$1);
                    if(modObj.length){
                         modObj.push(function(){
                             //检查模块是否已经大于loaded，如果是，则重新从缓存中绑定模块关系，否则无法回调使用！！
                             utils$1.bindModsRecursively(modNames,Zr$1);
                             //
                             //
                             finalCallback();
                         });
                         Zr$1.use.apply(this,modObj);
                        return;
                    }
                    //检查模块是否已经大于loaded，如果是，则重新从缓存中绑定模块关系，否则无法回调使用！！
                    if(modNames.length){
                    ...
                    waitingModules.fn = loadReady;
                    //检查模块是否已经载入并检测状态
                    modules = utils$1.checkAttachStatus(modules,Zr$1);
                    //
                    loader.use(modules);
                    // finalCallback();
                }
            ...
            function startAnalyse(waitingModules){
                if(!$.isEmptyObject(waitingModules.waitMods)){
                    utils$1.dependOnMod(waitingModules,Zr$1);
                }else{
                    //直接执行
                    waitingModules.notifyAll();
                }
            ...
            function reCheck(modNames,waitingModules){
                if(utils$1.checkIsLoadingStatus(modNames,Zr$1)){
            ...
            //第二步,开始分析并执行
            if($.isEmptyObject(Zr$1.global.MODULES)){
                checkWaiting(modNames,waitingModules);
            }else{
                reCheck(modNames,waitingModules);
            }
        ...
        ModulesAdd:function(name,factory,deps){
            var array = [],
                deps = deps || {},
                config = deps;
            array.push(Zr$1.global);
            deps = deps.requires || [];
            //分析name格式，将保留./name/这样形式的字符到requires
            deps = utils$1.registerNameAndAddRequireName(name,deps,Zr$1);
            ...
            //
            //
            config = utils$1.addQueenRequire(config,factory);
            //
            utils$1.registerModule(name,factory,config,Zr$1);
        },
        /**
        * 计算动态依赖模块加载
        * */
        calculate:function(modNames, cache, ret){
            ...
            $.each(modNames,function(i,n){
                ...
                cache[m] = 1;
                mod = utils$1.createModuleInfo(Zr$1,m);
                modStatus = mod.status;
...
//
Zr$1.global.document = Zr$1.document = document;
Zr$1.global.window = Zr$1.window = window;
Zr$1.global.baseConfig = Zr$1.baseConfig = config;
Zr$1.global.config = Zr$1.config = function(options){
    Zr$1.global.baseConfig = Zr$1.baseConfig = $.extend(true,Zr$1.global.baseConfig,options);
};
//添加完后执行别名执行操作
//Zr.global.use = Zr.use = modules.Modules;
Zr$1.global.use = Zr$1.use = ModulesObject.ModulesFix;
Zr$1.global.add = Zr$1.add = ModulesObject.ModulesAdd;
Zr$1.global.require = Zr$1.require = getFile;
Zr$1.global.tools = Zr$1.tools = $;
Zr$1.global.extend = Zr$1.extend = $.extend;
// Zr.global.dom = Zr.dom = dom;
// Zr.global.init = Zr.init = dom.init;
Zr$1.global.event = Zr$1.event = event;
//
Zr$1.global.readyQueen = Zr$1.readyQueen = [];
Zr$1.global.ready = Zr$1.ready = function(fn){
    console.log("first ready")
    Zr$1.readyQueen.push(fn);
};
Zr$1.tools.later(function(){
    console.log("ready load")
    Zr$1.use("zr",function(zr){
        Zr$1.tools.each(Zr$1.readyQueen,function(i,n){
            n();
        });
    });
},0);
