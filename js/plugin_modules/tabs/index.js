Zr.add("./tabs/index",function(zr,$){
    ...
    var tabs = {
        ...
        eventsFn:{
            ...
            styleInitFn:function(n){
                var $n = $(n),
                    ...
                    $header = $n.find("> .zr-tab-header"),
                    $content = $n.find(" > .zr-tab-content"),
                    $contentItems = $content.find("> .zr-tab-tabpane"),
                    $items = $header.find("> .zr-tab-item");
                var index = $items.index($header.find(".zr-tab-action"));
                if(index == -1){
                    index = 0;
                    $items.eq(index).addClass("zr-tab-action");
                }
                $contentItems.eq(index).addClass("zr-tab-show");
            ...
            bindSimpleFn:function(){
                var $this = $(this);
                if($this.hasClass("zr-tab-disable")){
                    return false;
                }
                var $tab = $this.closest(".zr-tab"),
                    $header = $this.closest(".zr-tab-header"),
                    $content = $tab.find(" > .zr-tab-content"),
                    $items = $header.find("> .zr-tab-item"),
                    options = $tab.data("options"),
                    $contentItems = $content.find("> .zr-tab-tabpane");
                //simple model just find content by index
                var index = $items.index($this),
                    contentClassName = "zr-tab-show",
                    headerClassName = "zr-tab-action";
            ...
            bindEditFn:function(){
                ...
                var tabClassName = "zr-tab-action",
                    contentClassName = "zr-tab-show";
                //
            ...
            reChooseFn:function(){
                var $this = $(this),
                    $header = $this.find("> .zr-tab-overflow > .zr-tab-header"),
                    $content = $this.find("> .zr-tab-content");
                var id = $this.data("tabOpts")|| {};
                id = id.id || "";
                var $items = $header.find("> .zr-tab-item"),
                    $panes = $content.find("> .zr-tab-tabpane");
                var tabClassName = "zr-tab-action",
                    contentClassName = "zr-tab-show";
                $items.removeClass(tabClassName);
            ...
            rebuildTabFn:function(config){
                ...
                $.each(config,function(i,n){
                    tabHtml += "<li class=\"zr-tab-item \" xid=\""+n.id+"\">"+n.title+"</li>";
                    contentHtml += n.url ? "<div class=\"zr-tab-tabpane\" xid='"+n.id+"' style=\"width:100%;height:300px;\"><iframe data-src=\""+n.url+"\"></iframe></div>" : "<div class=\"zr-tab-tabpane\" xid='"+n.id+"'>"+n.content+"</div>"
                })
                //
                //
                var $this = $(this),
            ...
            checkIdStatusFn:function(config){
                var _this = this;
                var $header = $(_this).find("> .zr-tab-overflow > .zr-tab-header");
                var rets = [],
                    ret;
                if(!zr.tools.isArray(config)){
                    rets.push(config)
                }else{
                    rets = config;
                }
                ...
                $.each(rets,function(i,n){
                    ret = $header.find("> .zr-tab-item[xid='"+n.id+"']");
                    if(ret.length == 0){
                        _array.push(n);
                    }
                    if(n.isSelect){
            ...
            checkScrollFn:function(target){
                var $target = $(target),
                    ...
                    $overflow =  $target.find("> .zr-tab-overflow"),
                    $header = $overflow.find("> .zr-tab-header"),
                    $left = $overflow.find("> .zr-tab-left"),
                    $right = $overflow.find("> .zr-tab-right"),
                    $content = $target.find(" > .zr-tab-content"),
                    $items = $header.find("> .zr-tab-item");
                //
                ...
                //
                if(_fixw <= 0){
                    $left.removeClass("zr-tab-show").removeClass("zr-tab-disable");
                    $right.removeClass("zr-tab-show").removeClass("zr-tab-disable");
                    $header.removeClass("zr-tab-mid");
                    return false;
                }
                var _ml = parseFloat($header.css("marginLeft") || 0);
                if(_ml == 0){
                    // left disable
                    $left.addClass("zr-tab-show").addClass("zr-tab-disable");
                    $header.addClass("zr-tab-mid");
                    $right.addClass("zr-tab-show").removeClass("zr-tab-disable");
                }else if(Math.abs(_ml) == Math.abs(_fixw)){
                    //right disable
                    $right.addClass("zr-tab-show").addClass("zr-tab-disable");
                    $header.addClass("zr-tab-mid");
                    $left.addClass("zr-tab-show").removeClass("zr-tab-disable");
                }else{
                    //both side active
                    $right.addClass("zr-tab-show").removeClass("zr-tab-disable");
                    $header.addClass("zr-tab-mid");
            ...
            menu:function(target){
                ...
                if(options.isMenu){
                    ...
                    $items.off("contextmenu").on("contextmenu",function(ev){
                        zr.fn.tabs.tab = this;
                        var _$items = $header.find("> .zr-tab-item");
                        if(ev.which == 3){
                            var x = ev.pageX,
                                y = ev.pageY;
                            ...
                            zr.fn.tabs.clickStatus = 1;
                            return false;
                        }
                    })
                    $("#ev-menu").unbind("mouseover").bind("mouseover",function(){
                        zr.fn.tabs.clickStatus = 0;
                    })
                    ...
                    $("#ev-menu li").unbind("click").bind("click",function(){
                        ...
                        if($(_this).hasClass("closeOthers")&&!$(_this).hasClass("disabled")){
                            //关闭其他
                            var $item = $(zr.fn.tabs.tab),
                                $header = $item.parent(),
                                $tab = $item.closest(".zr-tab"),
                                op = $tab.data("options"),
                                $content = $tab.find("> .zr-tab-content"),
                                $tabpanes = $content.find("> .zr-tab-tabpane"),
                                $items = $header.children(".zr-tab-item");
                            var xid;
                            if($item.length > 0){
                                ...
                                $others.each(function(){
                                    ...
                                    if(xid){
                                        ...
                                        var _$items = $header.children(".zr-tab-item"),
                                            _$tabpanes = $content.find("> .zr-tab-tabpane");
            ...
            closeBtnFn:function(target){
                var $target = $(target),
                    ...
                    $items = $header.find("> .zr-tab-item");
                if($items.length == 1){
                    $items.find(".zricon-close").remove();
                    return false;
                }
                //查找并自动添加关闭按钮
                if(options.isClose){
                    $items.each(function(){
                        var $close = $(this).find(".zricon-close");
                        if($close.length == 0){
                            $(this).append("<i class='zricon-close'></i>");
                        }
                    })
                }
                //
                var $closes = $items.find(".zricon-close");
                $closes.off("click").on("click",function(ev){
                    ev.stopPropagation();
                    var $p = $(this).parent(),
                        $target = $(this).closest(".zr-tab"),
                        $items = $(this).closest(".zr-tab-header").find(".zr-tab-item"),
                        xid = $p.attr("xid");
                    if(!xid){
        ...
        events:{
            init:function(options){
                var targets = zr.dom.find(options.target);
            ...
            reloadTabsEvent:function(target){
                var $n = $(target),
                    options = $n.data("options"),
                    $header = $n.find("> .zr-tab-overflow > .zr-tab-header"),
                    $content = $n.find(" > .zr-tab-content"),
                    $items = $header.find("> .zr-tab-item");
            ...
            updateEditEvent:function(config){
                ...
                var $tab = $(this.target);
                var $header = $tab.find("> .zr-tab-overflow > .zr-tab-header"),
                    $item = $header.find("> .zr-tab-item[xid='"+xid+"']"),
                    $content = $tab.find(" > .zr-tab-content"),
                    _id = $item.attr("xid");
                if(!xid){
                    return false;
                ...
                if(sp > -1){
                    ...
                    var $it = $header.find("> .zr-tab-item").eq(sp);
                    if($it.length > 0){
                        $it.click();
                    }
                }else{
            ...
            deleteEditEvent:function(xid){
                ...
                if(sp > -1){
                    sp = parseInt(xid.split("x_")[1]) - 1;
                    $item = $header.find("> .zr-tab-item").eq(sp);
                }
                ...
                if(sp > -1){
                    ...
                    $item = $header.find("> .zr-tab-item").eq(sp);
                    $item.remove();
                    $content.find("> .zr-tab-tabpane").eq(sp).remove();
                    //重载关闭方法
                ...
                if(xid){
                    //删除对应id
                    $item.remove();
                    $content.find("> .zr-tab-tabpane[xid='"+xid+"']").remove();
                }
                //
                //
                //
    ...
    zr.dom.clickQueen.push(function(){
        var $menu = $("#ev-menu");
        if($menu.length > 0 && zr.fn.tabs.clickStatus){
            $menu.hide();
        }
    })
    return {
    ...
    requires:["jquery","/tabs/css/index.css"]
})