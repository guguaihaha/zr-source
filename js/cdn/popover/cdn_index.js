Zr.add("./popover/cdn_index",function(Zr,$){
    var popover = {
        uid: 40600000,
        init: function(config) {
            ...
            $(".zr-popover-content").off('click').on('click',function(ev){
                ev.stopPropagation();
            })
        },
        options: {
    ...
    function Pop(config){
        ...
        this.optCont = this.optTarget.find(".zr-popover-content");
        this.optInner = this.optTarget.find(".zr-popover-inner");
        this.optBtn = this.optTarget.find('.zr-popover-btn');
        var _this = this;

        if(config.event == 'click'){
            $(Zr.document).on('click',function(){
                if(_this.optCont.css('display')=='block'){
                    _this.onClose && _this.onClose();
                }  
                $(".zr-popover-content").stop().fadeOut(300);          
            })
        }
    }
    ...
    Pop.prototype.popBtnFn = function() {
        ...
        this.optTarget.off(eventName).on(eventName,function(ev){
            ev.stopPropagation();
            //$(".zr-popover-content").stop().fadeOut(100);
...
},{requires:["jquery","/popover/css/cdn_index.css"]})